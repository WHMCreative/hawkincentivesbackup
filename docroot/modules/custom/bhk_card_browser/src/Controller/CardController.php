...
namespace Drupal\bhk_card_browser\Controller;
 ...
 */
class CardController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionShow($card_revision) {
    $card = $this->entityManager()->getStorage('card')->loadRevision($card_revision);
    $view_builder = $this->entityManager()->getViewBuilder('card');
  ...
  public function revisionPageTitle($card_revision) {
    $card = $this->entityManager()->getStorage('card')->loadRevision($card_revision);
    return $this->t('Revision of %title from %date', ['%title' => $card->label(), '%date' => format_date($card->getRevisionCreationTime())]);
  }

  /**
   * Generates an overview table of older revisions of a Card .
   *
   * @param \Drupal\bhk_card_browser\Entity\CardInterface $card
   *   A Card  object.
   *
   * @return array
   *   An array as expected by drupal_render().
   */
  public function revisionOverview(CardInterface $card) {
    $account = $this->currentUser();
    $langcode = $card->language()->getId();
    $langname = $card->language()->getName();
    $languages = $card->getTranslationLanguages();
    $has_translations = (count($languages) > 1);
    $card_storage = $this->entityManager()->getStorage('card');
    ...
    foreach (array_reverse($vids) as $vid) {
      /** @var \Drupal\bhk_card_browser\CardInterface $revision */
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($vid != $card->getRevisionId()) {
          $link = $this->l($date, new Url('entity.card.revision', ['card' => $card->id(), 'card_revision' => $vid]));
        }
        else {
          $link = $card->link($date);
        }
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              ...
              Url::fromRoute('entity.card.translation_revert', ['card' => $card->id(), 'card_revision' => $vid, 'langcode' => $langcode]) :
              Url::fromRoute('entity.card.revision_revert', ['card' => $card->id(), 'card_revision' => $vid]),
            ];
          }
          ...
          if ($delete_permission) {
            $links['delete'] = [
              'title' => $this->t('Delete'),
              'url' => Url::fromRoute('entity.card.revision_delete', ['card' => $card->id(), 'card_revision' => $vid]),
            ];
          }
